# 冻梨OJ综合开发日志（2026-01-06 ~ 2026-02-09）

## 📋 项目概述
**项目名称**：负载均衡在线OJ系统  
**开发周期**：2026年1月6日 - 2026年2月9日  
**主要模块**：用户认证系统、评测引擎、前端界面、系统架构、管理员系统、讨论区、竞赛系统  
**版本演进**：V0.2.1 → ... → V1.0.0 → V1.0.3
...
## 📅 2026-02-09 | V1.0.3 架构升级与爬虫重构
### 🎯 核心目标
完成洛谷爬虫的 C++ 重构以移除 Python 依赖，并将 OJ 服务默认端口迁移至 8088 以解决端口冲突问题。

### 🔧 关键变更
#### 1. 洛谷爬虫重构 (C++)
- **移除 Python**: 废弃原有的 `luogu_crawler.py` 脚本，彻底移除 Python 运行时依赖。
- **C++ 实现**: 新增 `crawler/luogu_crawler.cc`，基于 `libcurl` 和 `jsoncpp` 实现。
- **优势**: 统一技术栈，简化部署流程，利用 C++ 的高性能特性。

#### 2. 服务端口变更
- **端口迁移**: OJ 主服务器默认端口从 `8080` 变更至 `8088`。
- **文档更新**: 更新所有相关文档中的 URL 引用和启动命令。

### 📊 版本统计
- **标签状态**: ✅ V1.0.3
- **核心变更**: Python dependency removed, Port changed to 8088.

---

## 📅 2026-02-09 | V1.0.4 前端分页增强

### 🎯 核心目标
优化前端分页交互，提升用户浏览题目列表的体验。

### 🔧 主要改进
#### 1. 分页组件增强
- **导航扩展**：新增“首页”和“尾页”链接，方便用户快速切换。
- **快速跳转**：支持直接输入页码跳转，并隐藏了输入框的数字调节器（spinners）以保持界面整洁。

### 📊 版本统计
- **标签状态**: ✅ V1.0.4
- **核心变更**: Enhanced pagination with First/Last page links and direct page jump functionality.

---

## 📅 2026-02-09 | V0.3.7-fun 娱乐中心上线 (Entertainment Release)

### 🎯 核心目标
在严肃的算法训练之余，为用户提供一个放松身心的娱乐模块。本次更新重点引入了高质量的经典游戏复刻。

### 🔧 主要特性
#### 1. 超级玛丽 (Super Mario Bros) 重制版
- **引擎升级**: 弃用简易 JS 实现，引入基于 **Backbone Game Engine** 的完整物理引擎版本。
- **完美复刻**: 包含完整的 1-1 关卡设计、敌人AI（板栗仔/乌龟）、道具系统（蘑菇/花）及经典音效。
- **显示适配**: 
  - 实现 CSS `aspect-ratio: 256/240` 锁定 NES 原始比例。
  - 响应式布局：在宽屏和竖屏设备上自动最大化显示且不拉伸变形。
- **操作优化**: 
  - 重新映射键位：支持使用键盘方向键（←/→/↑）进行移动和跳跃，符合直觉。

### 📊 版本统计
- **标签状态**: ✅ V0.3.7-fun
- **核心变更**: Integrated Backbone Game Engine Mario, Fixed Aspect Ratio, Remapped Controls.
- **发布类型**: 🎮 娱乐特供版
...
## 📅 2026-02-22 | V1.1.0 题单与训练计划模块上线

### 🎯 核心目标
推出“题单/训练计划”功能，允许用户创建、分享和管理题目集合，支持拖拽排序和难度分级，打造结构化的刷题体验。

### 🔧 核心功能

#### 1. 题单管理系统
**后端实现**
- **数据库扩展**: 新增 `training_lists` 和 `training_list_items` 表。
- **CRUD 接口**: 实现题单的创建、编辑、删除、详情获取接口。
- **题目关联**: 支持向题单添加题目、移除题目。

#### 2. 交互体验升级
**拖拽排序**
- 引入 **SortableJS** 实现题目的拖拽排序。
- 后端提供 `reorder` 接口批量更新题目顺序。
...
### 📊 版本统计
- **标签状态**: ✅ V1.1.0
- **核心变更**: Training List Module, SortableJS Integration.
- **文档更新**: 全面更新 Database, API, README.

---

## 📅 2026-03-04 | V1.1.1 用户头像修复 (Avatar Fix)

### 🎯 核心目标
修复用户头像上传后无法即时更新的问题，优化会话同步机制。

### 🔧 关键修复
#### 1. 会话同步优化
- 修正了 `UploadAvatar` 和 `UpdateUserInfo` 中的逻辑缺陷，现在会遍历并更新用户的所有活跃会话（Active Sessions），而不仅仅是第一个匹配项。这解决了多端登录或多标签页下头像不一致的问题。

#### 2. 文件清理优化
- 引入标志位确保在更新多个会话时，旧头像文件仅被尝试删除一次，减少系统调用开销。

### 📊 版本统计
- **标签状态**: ✅ V1.1.1
- **核心变更**: Fix session update logic for avatar/profile changes.
...
## 📅 2026-03-09 | V1.1.3 首页与导航栏重构

### 🎯 核心目标
重构首页 UI/UX，统一全站导航栏风格，提升首屏加载性能。

### 🔧 主要改进
#### 1. 导航栏重构
- **公共组件化**：全站统一使用 `navbar.html` 组件，消除不同页面间导航栏的视觉差异。
- **状态同步**：修复了首页用户头像与登录状态与其他页面不一致的 Bug，确保用户体验连贯性。

#### 2. 首页性能优化
- **路由直出**：首页 (`/`) 移除重定向逻辑，改为服务端直接渲染，减少一次 HTTP 跳转，显著提升首屏加载速度（LCP）。

### 📊 版本统计
- **标签状态**: ✅ V1.1.3
- **核心变更**: Homepage refactoring, Navbar unification, Performance boost.
...
## 📅 2026-03-09 | V1.2.0 文档全面同步 (Documentation Sync)

### 🎯 核心目标
全面校对和更新项目文档，确保技术文档与代码库的实际状态（API、数据库、架构、部署）完全一致。

### 📝 文档更新详情
- **API 接口文档**: 补充了题单、社区讨论、文章评论、管理员管理等所有缺失接口，更新了请求参数和响应示例。
- **数据库文档**: 补全了 `training_lists`, `discussions`, `inline_comments` 等所有新表的结构定义和索引设计。
- **架构文档**: 更新了模块设计图，补充了题单系统和社区系统的架构描述。
- **部署文档**: 确认了 Docker 部署流程，统一了文档版本号。
- **README**: 修正了服务端口号 (8088 -> 8094)，更新了版本历史。
...
## 📅 2026-03-10 | V1.3.1 编译器错误信息修复

### 🎯 核心目标
修复编译器启动失败时报错含糊不清的问题，并增强对编程语言名称的容错处理。

### 🔧 关键修复
- **编译器启动优化**: 修正 execlp 参数哨兵为 (char*)0，修复 Invalid argument 错误；重定向启动错误至 stderr，使 "g++: command not found" 等错误可见。
- **语言名称标准化**: 后端 compile_run 模块新增对 cpp/c++/cc, java, python/py 等别名的自动映射，解决了前端传参小写导致判题失败的问题。
- **明确报错**: 当语言不支持时返回明确的 "Unsupported language" 错误而非系统错误。
...
## 📅 2026-03-11 | V2.0.0 前端重构 (React Evolution)

### 🎯 核心目标
完成前端技术的全面现代化升级，从传统的服务端渲染 (CTemplate) 转向 React SPA 架构，提升交互体验与开发效率。

### 🔧 技术决策
- **框架选型**: React 19 + Vite + TypeScript。选用 Vite 是因为其极速的冷启动和 HMR 体验；TypeScript 提供类型安全，减少运行时错误。
- **UI 组件库**: 采用 Shadcn UI (基于 Radix UI) + Tailwind CSS。这种组合提供了高度可定制的无头组件和原子化样式，避免了传统 UI 库的样式覆盖难题。
- **状态管理**: 选用 Zustand。相比 Redux，它更轻量、样板代码更少，且易于处理异步 Action (如登录、获取用户信息)。
- **路由方案**: React Router v7，支持嵌套路由和受保护路由 (ProtectedRoute)。

### 🏗️ 架构变更
- **目录结构**: 遵循标准的 `src/components`, `src/pages`, `src/hooks` 结构。
- **API 层**: 封装 Axios 拦截器，统一处理 401 状态码（自动跳转登录）和全局错误提示。
- **代理配置**: Vite 开发服务器配置 Proxy，解决前后端分离开发时的跨域问题。

### 🚧 踩坑记录
- **Avatar 更新**: 头像上传后，因浏览器缓存导致图片不更新。解决方案：在 URL 后追加时间戳参数 `?t=...`。
- **路由重定向**: SPA 部署到 Nginx/后端静态服务时，刷新非根路径会 404。解决方案：后端需配置 Fallback，将所有未匹配请求重定向至 `index.html`。
- **类型定义**: 早期未严格定义 API 响应类型，导致前端解析出错。后续补全了 `types/*.ts` 定义。

### 📊 性能优化
- **构建产物**: 使用 Vite 的 splitChunks 分割代码，Vendor (React/ReactDOM) 单独打包，利用浏览器长缓存。
- **加载速度**: 首页 LCP 从 1.2s 优化至 0.8s (本地测试)。

---

## 📅 2026-03-12 | V2.0.1 前端布局修复与公式渲染

### 🎯 核心目标
修复 React 重构后发现的布局问题，完善 Markdown 的数学公式渲染支持，提升阅读体验。

### 🔧 关键修复
#### 1. Markdown 公式渲染
- **引入 KaTeX**: 集成 `rehype-katex` 和 `remark-math` 插件。
- **样式修复**: 引入 `katex/dist/katex.min.css`，解决公式显示错乱问题。
- **组件封装**: 在 `MarkdownViewer` 组件中配置插件链，支持行内公式 (`$E=mc^2$`) 和块级公式 (`$$...$$`)。

#### 2. 全局布局优化
- **容器居中**: 调整 `Layout` 组件，使用 `max-w-7xl mx-auto` 约束最大宽度并居中，防止在大屏设备上内容过于分散。
- **导航栏修复**: 修复了导航栏在不同屏幕尺寸下的布局抖动问题，确保 Logo、导航链接和用户菜单对齐。

### 📊 版本统计
- **标签状态**: ✅ V2.0.1
- **核心变更**: Markdown Math Support, Layout Fixes.
...
## 📅 2026-03-12 | V2.0.2 题库分页增强

### 🎯 核心目标
优化题库列表的分页交互体验，使用户能更便捷地浏览大量题目。

### 🔧 主要改进
#### 1. 分页组件增强
- **首尾页导航**：新增“第一页”和“最后一页”按钮，支持一键跳转到列表的开头或结尾。
- **快速跳转**：新增页码输入框，允许用户直接输入页码进行跳转，提升长列表的导航效率。
- **布局优化**：将分页控件整体居中显示，视觉效果更加平衡和美观。

### 📊 版本统计
- **标签状态**: ✅ V2.0.2
- **核心变更**: Enhanced pagination controls (First/Last, Jump Input, Centered Layout).
