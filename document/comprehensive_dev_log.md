...
## 📅 2026-01-31 | V0.4.0 竞赛页面重构

### 🎯 核心目标
重构竞赛列表页面，采用现代化表格布局，支持 Codeforces 竞赛数据展示，并优化视觉体验。

### 🔧 主要特性

#### 1. 竞赛页面重构
**前端实现**
- **列表视图**: 将原有的 Grid 布局替换为响应式表格布局 (`<table>`)，提升信息展示密度。
- **状态管理**: 新增竞赛状态标签（未开始/进行中/已结束），基于时间自动计算。
- **视觉优化**: 引入 Codeforces Logo，优化标题栏样式；支持深色模式。
- **响应式设计**: 移动端自动切换为卡片式视图，适配不同屏幕尺寸。

**后端支持**
- **数据处理**: 新增 `contest_utils.hpp` 处理竞赛数据的排序和状态计算。
- **排序逻辑**: 实现按开始时间升序排列，相同时间按结束时间排序。

#### 2. 代码库优化
- **CSS 模块化**: 新增 `contest-list.css` 独立管理竞赛页面样式。
- **单元测试**: 新增 `test_contest_sort.cc` 验证排序算法的正确性。

### 📊 版本统计
- **标签状态**: ✅ V0.4.0
- **核心变更**: 竞赛页面 UI/UX 重构，排序逻辑优化。
